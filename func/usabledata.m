...
%% Load ERP file
    ...
    for iSubj=1:size(erp.erpAll,1)
        for iTrig = 1:size(erp.trig(1).triggers,1)
            % Get Proportion of usable Data for current subject and trigger
            if ~isempty(erp.corrTrials{iSubj})
    ...
    if ~isempty(threshold)
        ...
        for iSubj=1:size(erp.erpAll,1)
            if ~isempty(erp.corrTrials{iSubj})
                usableProp = [usableProp round(mean(corrProp(iSubj,:))*1000)/10];
            end
        end
        usableProp =  sort(usableProp,2,'descend')
        figure;
        bar(usableProp)
    end
end
